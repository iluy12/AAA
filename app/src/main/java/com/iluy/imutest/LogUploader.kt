...
object LogUploader {
    ...
    /**
     * מעלה טקסט כלשהו ומחזיר את הכתובת. משמש לייצוא הנתונים.
     *
     * ⚠️ **בלי גזירה לפי שורות או בייטים.** הקיצוץ קיים כדי שהלוג ישרוד
     * חיבור איטי, אבל ייצוא חתוך הוא ייצוא חסר-ערך — הניתוח דורש את
     * הסדרה השלמה. אם ההעלאה תיכשל על גודל, נדע מהתוצאה.
     */
    fun uploadText(body: String): String =
        try {
            if (body.isBlank()) "אין נתונים" else postWithFallbacks(body)
        } catch (e: Exception) {
            "שגיאה: ${e.javaClass.simpleName}"
        }
