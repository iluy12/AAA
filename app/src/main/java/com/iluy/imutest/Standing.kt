...
object Standing {
    ...
    /**
     * ⚠️ **המרת תשובת השאלון למספר, ובכוונה בצד הנמוך של כל טווח.**
     *
     * "כמה חודשים" הופך ל-75 ולא ל-120: הערכת-יתר של השיא האישי הופכת כל
     * רצף אמיתי ל"עוד רחוק מהשיא", והמערכת לעולם לא תגיד לו שהוא קרוב.
     * הערכת-חסר גורמת לו לשמוע את זה מוקדם מדי — וזו הטעות הזולה מהשתיים.
     */
    fun longestStreakToDays(answer: String?): Int = when (answer) {
        "כמה ימים" -> 4
        "שבוע" -> 7
        "שבועיים" -> 14
        "חודש" -> 30
